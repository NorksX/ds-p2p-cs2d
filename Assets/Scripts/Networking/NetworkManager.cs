...
/// <summary>
/// Core network manager - handles all P2P connections using LiteNetLib
/// </summary>
public class NetworkManager : MonoBehaviour, INetEventListener
{
    ...
    // Peers
    private Dictionary<int, PeerInfo> connectedPeers = new Dictionary<int, PeerInfo>(); // Key: NetPeer.Id
    ...
    public bool HostLobby()
    {
        ...
        {
            isHost = true;
            nextPlayerPosition = 1; // Host takes position 0, clients start at 1
    ...
    public void OnPeerConnected(NetPeer peer)
    {
        ...
        if (isHost)
        {
            // Host: Accept the connection and send response
            ...
            // For now, auto-accept (we'll get username from JoinLobbyRequest)
            // We'll create the PeerInfo when we receive the JoinLobbyRequest message
        }
        else
        {
    ...
    public void OnPeerDisconnected(NetPeer peer, DisconnectInfo disconnectInfo)
    {
        ...
        // If we were client and host disconnected, we're disconnected
    ...
    public void OnConnectionRequest(ConnectionRequest request)
    {
        if (isHost)
        {
            // Accept all connections (we'll validate in OnPeerConnected)
            request.Accept();
        }
        else
        {
    ...
    public void BroadcastGameStart(List<PlayerSpawnInfo> players)
    {
        ...
        // Host creates the start message
        ...
        // Also trigger it locally for the host
    ...
    {
        // Clients receive this from the host
